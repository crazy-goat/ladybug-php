...
dnl Default yes, which is the convention for an extension that lives outside php-src: PIE runs
dnl `./configure` with no arguments at all unless the installer passes a declared option, so a
dnl default of `no` would make `pie install` build nothing and exit successfully.
PHP_ARG_ENABLE([ladybug],
  [whether to enable LadybugDB support],
  [AS_HELP_STRING([--enable-ladybug],
    [Enable LadybugDB support (default: yes)])],
  [yes])
